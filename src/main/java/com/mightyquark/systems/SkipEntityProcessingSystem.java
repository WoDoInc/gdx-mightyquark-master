...
import com.mightyquark.utils.Timer;
 ...
 * 
 * An example system would be an ExpirationSystem, that deletes entities after a certain lifetime. Another example system would be an
 * AnimationSystem. You know when you have to animate a certain entity, e.g. in 300 milliseconds. So you can set the system to run in
 * 300 ms. to perform the animation. This will save CPU cycles in some scenarios.
 * 
 * Note, this system stores Entity state and thus is not true to the Entity System priciples. This was done to avoid storing extra
 * information on Entities whether in form of components or otherwise. Implementing an EntitySystem in this manner should generally
 * be avoided.
 */
